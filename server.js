...
app.get("/keepa", async (req, res) => {
  ...
  try {
    const keepaURL = `https://api.keepa.com/product?key=${KEEPA_API_KEY}&domain=1&asin=${asin}&history=1`;

    const response = await fetch(keepaURL);
    const data = await response.json();

    if (!data.products || data.products.length === 0) {
      return res.status(404).json({ error: "No product data returned by Keepa", raw: data });
    }
